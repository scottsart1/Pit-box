...
Use `strategy-independent-validation-3ed73b9.md` and the committed comparison JSON. The same evaluator, frozen worlds, observations and exact oracles are applied to both baseline and candidate. Two adapter mistakes were corrected openly and both revisions rerun: EA remaining-life versus maximum-stint semantics, and end-of-lap boxing versus beginning-of-lap oracle actions.

Preliminary verified result: all 24 candidate recommendations are physically executable, versus 20 baseline recommendations. In the 20 jointly executable worlds, four improve and 16 are unchanged. The paired median regret is unchanged; feasibility gains must not be presented as broad pace calibration. Weather, nonlinear wear, unobserved rival response and removed-set refitting retain explicit limitations. Candidate runtime is higher and is measured separately.

## App/storage gate

Two fresh 25-lap accelerated source-app replays on the workspace filesystem encountered SQLite corruption and a session API error. Original evidence directories are `pitbox-strategy-e2e-mrmk8g2z` and `pitbox-strategy-e2e-ljt7eqb2`. Read-only copies confirmed failed integrity checks. A shorter replay and a 25-lap memory-filesystem control completed without storage corruption. Investigation is ongoing; this is a release blocker, not proof of a particular root cause or a Windows failure.
