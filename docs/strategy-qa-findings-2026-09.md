...
| Weather and finite inventory | An immediate WET call replaced a feasible WET/WET two-stop plan with an infeasible one-stop. Its special path could also use an already-passed pit entry. Selection now preserves an executable complete physical-set schedule and the next reachable entry. | `test_strategy_weather_inventory.py`; four failing cases before, six passing cases after |
...
| Capture rotation | During awaited catalog registration, all 202 test datagrams were rejected silently while queue-drop and write-error counters stayed zero. The paired fixed probe persists 208/208, including all 202 during registration; overflow is bounded and counted. | `test_capture_rotation_handoff.py`, `capture-rotation-observations.md` and paired probe reports |
| Capture shutdown/error reporting | A zero-timeout stop left rotation futures unresolved; writer-open failure did not increment write errors. Pending command tasks now settle, owned file operations are reclaimed and rotation failures are counted. | Capture handoff timeout/open-failure regressions |
