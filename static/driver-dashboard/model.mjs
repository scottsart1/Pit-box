...
export function normalizeState(s,{ageMs=0,transport='live',now=Date.now()}={}) {
  ...
  const sessionData=fresh&&freshGroup('1'),motionData=fresh&&freshGroup('0'),lapData=fresh&&freshGroup('2');
  ...
  return {fresh,transport,provenance,paused:!!s.game_paused,flag,track:String(s.track_name||'Waiting for session'),session:String(s.session_type||'No session'),
    ...
    lateral:motionData?finite(s.lateral_g):null,longitudinal:motionData?finite(s.longitudinal_g):null,
...
export function demoState(scenario='green',tick=0) {
  ...
  Object.assign(state,{throttle:.92,brake:0,steer:-.08,lateral_g:-1.4,longitudinal_g:.62,weather:'Light cloud',session_time_left_s:3042,track_length_m:5891,lap_distance_m:4806,unserved_drive_through_penalties:0,unserved_stop_go_penalties:0,drs_allowed:true,overtake_available:true,ers_deployed_lap_j:1800000,ers_harvested_lap_j:2200000,damage:{front_left_wing:2,front_right_wing:0,rear_wing:0,floor:1,diffuser:0,sidepod:0,gearbox:4,engine:3},component_wear:{ice:18,tc:14,mguk:12,mguh:11,es:8,ce:6}});
  Object.assign(state.tyre,{inner_temps_c:[99,100,97,98],pressures_psi:[23.8,23.9,21.3,21.4]});
  ...
  else if(scenario==='red'){state.race_control_phase='red_flag';state.speed_kph=0;state.gear=0;state.throttle=0;state.brake=.65;state.engine_rpm=4500;state.lateral_g=0;state.longitudinal_g=0;state.active_aero_mode=0}
  else if(scenario==='sc'||scenario==='vsc'){state.race_control_phase=scenario==='sc'?'safety_car':'vsc';state.safety_car_delta_valid=true;state.safety_car_delta_s=.85;state.speed_kph=130;state.gear=4;state.throttle=.35;state.engine_rpm=8300;state.lateral_g=-.4;state.longitudinal_g=.1;state.active_aero_mode=0}
  else if(scenario==='penalty')state.unserved_drive_through_penalties=1;
  else if(scenario==='pit'){state.pit_status=2;state.pit_speed_limit_kph=80;state.speed_kph=79;state.gear=2;state.throttle=.3;state.engine_rpm=7200;state.lateral_g=0;state.longitudinal_g=0;state.active_aero_mode=0}
  else if(scenario==='finish'){state.fia_flag='chequered';state.current_lap=52;state.speed_kph=0;state.gear=0;state.throttle=0;state.engine_rpm=4500;state.lateral_g=0;state.longitudinal_g=0;state.active_aero_mode=0}
  return state;
}
