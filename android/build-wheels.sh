...
# itself), and uv. Versions in native-requirements.txt also constrain the
# APK's resolver, because pydantic pins pydantic-core exactly.
set -euo pipefail
...
while IFS= read -r entry || [ -n "$entry" ]; do
  ...
  if "$PYTHON" "$HERE/check-wheels.py" --package "$entry" --archs $CIBW_ARCHS >/dev/null; then
    echo "== $name $version: already built"
    continue
  fi
  echo "== $name $version"
  url="$(curl --fail --silent --show-error "https://pypi.org/pypi/$name/$version/json" \
  ...
  [ -f "$sdist" ] || curl --fail --silent --show-error --location -o "$sdist" "$url"
  rm -rf "$WORK/$name" && mkdir -p "$WORK/$name"
  tar xzf "$sdist" -C "$WORK/$name"
  ( cd "$WORK/$name"/* && "$PYTHON" -m cibuildwheel --output-dir "$OUT" . )
done < "$HERE/native-requirements.txt"

"$PYTHON" "$HERE/check-wheels.py" --archs $CIBW_ARCHS
